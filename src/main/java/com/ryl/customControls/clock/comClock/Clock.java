...
package com.ryl.customControls.clock.comClock;

import com.ryl.customControls.clock.comClock.skin.ClockSkin;
...
public class Clock extends Control {
    ...
    private Duration clamp(Duration min, Duration max, Duration value) {
        if (getDateTime().plus(value).isBefore(getDateTime().plus(min))) {
            return min;
        }
        if (getDateTime().plus(value).isAfter(getDateTime().plus(max))) {
            return max;
        }
        return value;
    }
    ...
    @Override public String getUserAgentStylesheet() {
        return getClass().getResource("/clock/clock.css").toExternalForm();
    }
}
