...
package com.ryl.customControls.gauge.markerGauge;

import com.sun.javafx.css.converters.PaintConverter;
import com.ryl.common.Marker;
import com.ryl.common.Section;
import com.ryl.customControls.gauge.markerGauge.skin.GaugeSkin;
...
public class Gauge extends Control {
        ...
        minValue                 = new DoublePropertyBase(0) {
            @Override protected void invalidated() {
                ...
                for (Marker marker : markers.keySet()) {
                    if (marker.getValue() < get()) {
                        marker.setValue(get());
                    }
                }
                for (Section section : sections) {
                    if (section.getStart() < get()) {
                        section.setStart(get());
                    }
                    if (section.getStop() < get()) {
                        section.setStop(get());
                    }
                }
            }
    ...
    public String getUserAgentStylesheet() {
        return getClass().getResource("/gauge/marketGauge/gauge.css").toExternalForm();
    }
